...
const HighveldEnvironment = ({
  ...
  // Static tiers have no running loop, so camera changes need an explicit
  // repaint or the canvas keeps showing the overview.
  ...
  useEffect(() => {
    ...
    const animates =
      ...
      resolvedQuality !== "low";
    const minFrameGap = resolvedQuality === "medium" ? 1000 / 30 : 0;
    ...
    const tick = (timestamp: number): void => {
      if (timestamp - lastMark >= minFrameGap) {
        paint(timestamp);
        lastMark = timestamp;
      }
    ...
    let disposed = false;
    repaintRef.current = animates ? null : (): void => paint(fixedTimestamp ?? 0);
    ...
    const handleResize = (): void => {
      resize();
      if (!animates) {
        // Static tiers still need a repaint or the canvas clears on resize.
        paint(fixedTimestamp ?? 0);
      }
    };
